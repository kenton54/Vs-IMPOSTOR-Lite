...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		#if (LUA_ALLOWED || HSCRIPT_ALLOWED)
		for (file in Assets.readDirectory(Paths.getLitePath('scripts/')))
		{
			#if LUA_ALLOWED
			if (Path.extension(file) == 'lua')
				startLuasNamed(file, true);
			#end

			#if HSCRIPT_ALLOWED
			if (Path.extension(file) == 'hx')
				startHScriptsNamed(file, true);
			#end
		}
		#end
		...
		#if (LUA_ALLOWED || HSCRIPT_ALLOWED)
		for (file in Assets.readDirectory(Paths.getLitePath('data/songs/$songName/')))
		{
			#if LUA_ALLOWED
			if (Path.extension(file) == 'lua')
				startLuasNamed(file, true);
	...
	public function startVideo(name:String, forMidSong:Bool = false, canSkip:Bool = true):VideoSprite
	{
		...
		if (forMidSong)
			videoCutscene.speed = playbackRate;
	...
	override public function update(elapsed:Float)
	{
		...
		if ((controls.PAUSE #if mobile
			|| (overlapsPauseButton && PointerUtil.justReleased) #if android || FlxG.android.justReleased.BACK #end #end)
			&& startedCountdown
			&& canPause)
		{
	...
	function updateTimeBar()
	{
		...
		if (ClientPrefs.data.timeBarType == TimeElapsed
			|| ClientPrefs.data.timeBarType == TimeLeft /*|| ClientPrefs.data.timeBarType == TimeTotal*/)
		{
	...
	function doDeathCheck(?skipHealthCheck:Bool = false)
	{
		if (((skipHealthCheck && instakillOnMiss) || health <= 0) && !practiceMode && !isDead)
		{
			...
			if (ret != LuaUtils.Function_Stop)
			{
				...
				for (cam in [camHUD, camOther, camCountdown, camDialogue #if mobile, camControls #end])
					cam.visible = false;
				persistentUpdate = false;
		...
		switch (eventName)
		{
			...
			case 'Add Camera Zoom':
				if (ClientPrefs.data.camZooms /* && FlxG.camera.zoom < 1.35*/)
				{
					...
					mainCamTwn = FlxTween.tween(FlxG.camera, {zoom: FlxG.camera.zoom + flValue1}, 0.12, {
		...
		if (SONG.notes[curSection] != null)
		{
			...
			if (camZooming /* && FlxG.camera.zoom < 1.35*/ && ClientPrefs.data.camZooms)
			{
				...
				if (mainCamTwn != null)
					mainCamTwn.cancel();
				camZoomingBeat = false;
				mainCamTwn = FlxTween.tween(FlxG.camera, {zoom: FlxG.camera.zoom + 0.015 * camZoomingMult}, 0.12, {
					ease: FlxEase.cubeOut,
					onComplete: function(_)
					{
						camZoomingBeat = true;
