...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	function addSongUI():Void
	{
		...
		for (file in Assets.readDirectory(Paths.getLitePath('characters/')))
		{
		...
		var stageFile:Array<String> = Mods.mergeAllTextsNamed('data/stageList.txt', Paths.getLitePath());
		...
		for (file in Assets.readDirectory(Paths.getLitePath('stages/')))
		{
			if (file.endsWith('.json'))
			{
	...
	function addEventsUI():Void
	{
		...
		for (file in Assets.readDirectory(Paths.getLitePath('events/')))
		{
			if (file.endsWith('.txt'))
			{
				...
				if (!eventPushedMap.exists(fileToCheck))
				{
					...
					eventStuff.push([fileToCheck, Assets.getText(file)]);
				}
			}
		}
	...
	function loadCharacterFile(char:String):CharacterFile
	{
		...
		var path:String = Paths.getLitePath('characters/$char.json');
