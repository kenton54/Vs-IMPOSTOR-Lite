...
class TextFunctions
{
	public static function implement(funk:FunkinLua)
	{
		...
		Lua_helper.add_callback(lua, "makeLuaText", function(tag:String, text:String, width:Int, x:Float, y:Float) {
			...
			leText.setFormat(Paths.font("vcr"), 16, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		Lua_helper.add_callback(lua, "setTextAlignment", function(tag:String, alignment:String = 'left') {
			...
			if(obj != null)
			{
				...
				switch(alignment.trim().toLowerCase())
				{
					...
					case 'justify': obj.alignment = JUSTIFY;
				}
				return true;
			}
	...
	static function getTextObject(tag:String):Null<FlxText>
	{
		...
		return split.length > 1 ? (LuaUtils.getVarInArray(LuaUtils.getPropertyLoop(split), split[split.length - 1])) : LuaUtils.getObjectDirectly(split[0]);
	}
}
#end